  ...
  out$.novelty = novelty = async function(enabled_goals){
    ...
    for (intervention_name in enabled_interventions) {
      ...
      for (i$ = 0, len$ = (ref$ = intervention.goals).length; i$ < len$; ++i$) {
        goal_name = ref$[i$];
        if (goal_name in enabled_goals && (novel_interventions[goal_name] == null || time === -1 || (novel_interventions[goal_name].time !== -1 && novel_interventions[goal_name].time < time))) {
